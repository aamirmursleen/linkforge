...
// GET /api/redirect/custom - Handle custom domain redirects
export async function GET(request: NextRequest) {
  try {
    ...
    const customDomain = await prisma.domain.findUnique({
    ...
    // Find short link for this domain
    ...
    if (!link) {
      // Check for custom 404 URL
      if (customDomain.notFoundUrl) {
        return NextResponse.redirect(customDomain.notFoundUrl, { status: 302 });
      }
    ...
    if (!statusCheck.canAccess) {
      ...
      if (statusCheck.status === "password_required") {
        ...
        if (!sessionToken || !verifyPasswordSessionToken(sessionToken, link.id)) {
          ...
          return NextResponse.redirect(`${baseUrl}/p/${code}?domain=${domain}`, { status: 302 });
        }
      } else {
        return NextResponse.json(
